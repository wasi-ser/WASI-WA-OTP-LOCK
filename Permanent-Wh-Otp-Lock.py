...
//          //..\\\
       ...
       ====UU====UU====
           '\/\/'
[bold {color}]●[bold {colors[(colors.index(color) + 1) % len(colors)]}] ●[bold {colors[(colors.index(color) + 2) % len(colors)]}] ======================================================
[bold white][[bold red]^[bold white]] [bold green] Author: MR WASI TECH  \n[bold white][[bold red]^[bold white]] [bold green] Github: github.com/wasi-ser\n[bold white][[bold red]^[bold white]] [bold green] Telegram: https://t.me/+GrRkWxyiROs4ZGU1
[bold {color}] [bold {colors[(colors.index(color) + 1) % len(colors)]}] [bold {colors[(colors.index(color) + 2) % len(colors)]}]===================================================== ''', title="[bold red] Created By MR WASI", style=color)
        console.print(panel)
        time.sleep(0.5)
...
def main():
    otp_lock_banner()
    country_code = input("\n\033[90m[\033[91m?\033[90m]] \033[92m[X]Enter Your Country Code (e.g., +92): " '\n └─> ')
...
lines = [
    ("CALL ME WASI..", "93", True),
    ("I AM FROM PAKISTAN..", "92", True),
    ("I AM THE OWNER OF WASI TECH TEAM..", "94", True),
    ("HOPE YOU LIKE THIS SCRIPT..", "95", True),
    ("OOPS... I TALK A LOT SRY FOR THAT..", "96", True),
    ("Telegram Channel: ", "97", True),
    ("https://t.me/wasitechx", "91", False),
